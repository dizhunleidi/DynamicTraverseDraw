...
struct timer {
    ...
    // 计时器开始
    inline void start() { clock_gettime(CLOCK_MONOTONIC, &str); }
    ...
    inline void SetFps(unsigned int fps) { Fps = 1000000000 / fps; }

    inline void FpsEnd() { fpsio = false; }
    ...
    long long oldtimer;
    // 调用 AotuFPS 之前 请调用 AotuFPS_init  和 SetFps， SetFps
    // 可实时调整FPS。此函数建议放在循环最前面。函数返回值为当前循环时间，单位毫秒， 1000.0f/返回值可得到准确fps
    ...
    inline void looptimeend() { islooptimestart = false; }

    inline long getlooptime() {
        clock_gettime(CLOCK_MONOTONIC, &endloop);
        looptime =
            (((1000000000 * endloop.tv_sec) + (endloop.tv_nsec)) - ((1000000000 * strloop.tv_sec) + (strloop.tv_nsec)));
        return looptime;
    }
